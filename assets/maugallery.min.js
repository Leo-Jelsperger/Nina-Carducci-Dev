...
let index=0;for(let i=0;i<imagesCollection.length;i++){if($(activeImage).attr('src')===$(imagesCollection[i]).attr('src')){index=i;break}}
index=index-1;if(index<0){index=imagesCollection.length-1}
$('.lightboxImage').attr('src',$(imagesCollection[index]).attr('src'))},nextImage(){let activeImage=null;$('img.gallery-item').each(function(){if($(this).attr('src')===$('.lightboxImage').attr('src')){activeImage=$(this)}});let activeTag=$('.tags-bar span.active-tag').data('images-toggle');let imagesCollection=[];if(activeTag==='all'){$('.item-column').each(function(){if($(this).children('img').length){imagesCollection.push($(this).children('img'))}})}else{$('.item-column').each(function(){if($(this).children('img').data('gallery-tag')===activeTag){imagesCollection.push($(this).children('img'))}})}
let index=0;for(let i=0;i<imagesCollection.length;i++){if($(activeImage).attr('src')===$(imagesCollection[i]).attr('src')){index=i;break}}
index=index+1;if(index>=imagesCollection.length){index=0}
$('.lightboxImage').attr('src',$(imagesCollection[index]).attr('src'))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
